...
const aspectRatioDisplayNames: Record<AspectRatio, string> = {
  ...
  [AspectRatio.PORTRAIT]: 'TikTok/Reels (9:16)',
};
...
// Templates for the SaaS niche
const AD_TEMPLATES = [
  {
    label: '✨ Select a Winning Template...',
    prompt: '',
  },
  {
    label: '💊 Supplement Bottle Reveal (3D)',
    prompt:
      'Cinematic close-up of a premium supplement bottle sitting on a marble countertop. Golden hour lighting, dust particles floating in the air. The camera slowly orbits the bottle. 4k resolution, hyper-realistic commercial product shot, high fidelity, advertising aesthetics.',
  },
  {
    label: '🤳 UGC Selfie Testimonial',
    prompt:
      'A relatable young woman holding a product close to the camera, talking excitedly. Shot on a smartphone, vertical format, authentic user generated content style. Home bedroom background, natural lighting, casual vibe, tiktok aesthetic, high resolution.',
  },
  {
    label: '📦 TikTok Shop Unboxing',
    prompt:
      'POV shot of hands tearing open a cardboard package on a desk to reveal a new product. Fast movement, energetic vibe, bright ring light lighting. Authentic unboxing experience, viral tiktok shop style, 60fps.',
  },
  {
    label: '🌿 Organic Ingredients (Macro)',
    prompt:
      'Extreme macro shot of natural herbal ingredients falling into a pile. Slow motion, depth of field, vibrant colors. Fresh mint leaves, powders, natural lighting. High-end health brand aesthetic.',
  },
  {
    label: '👟 Fashion/Fit Check (Mirror)',
    prompt:
      'A person doing a fit check in a full length mirror. Trendy outfit, cool pose. Modern apartment background. Authentic social media style, high definition.',
  },
];
  ...
  <div>
    ...
    <div className="relative">
      ...
      <select
        ...
        onChange={onChange}
        disabled={disabled}
        className="w-full bg-[#1f1f1f] border border-gray-600 rounded-lg pl-10 pr-8 py-2.5 appearance-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 disabled:bg-gray-700/50 disabled:border-gray-700 disabled:text-gray-500 disabled:cursor-not-allowed">
        {children}
      </select>
      <ChevronDownIcon
    ...
    return (
      <div className="relative w-28 h-20 group">
        <img
          ...
          alt="preview"
          className="w-full h-full object-cover rounded-lg border border-emerald-500/50"
        />
        <button
          type="button"
          onClick={onRemove}
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  // Default to High Quality model for "Hyper Realistic" requirement
  const [model, setModel] = useState<VeoModel>(
    initialValues?.model ?? VeoModel.VEO,
  );
  // Default to Portrait for TikTok/Shorts focus
  ...
  const handleTemplateSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const selected = AD_TEMPLATES.find((t) => t.label === e.target.value);
  ...
  const promptPlaceholder = {
    [GenerationMode.TEXT_TO_VIDEO]: 'Describe your ad creative concept...',
    [GenerationMode.FRAMES_TO_VIDEO]:
      'Describe movement (e.g., "Camera zooms into the bottle")...',
    [GenerationMode.REFERENCES_TO_VIDEO]:
      'Describe the scene using reference images...',
    [GenerationMode.EXTEND_VIDEO]: 'Describe the next scene in the ad...',
  }[generationMode];
    ...
    if (generationMode === GenerationMode.FRAMES_TO_VIDEO) {
      return (
        <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-col items-center justify-center gap-4">
           <div className="text-sm text-gray-400 mb-2">Upload product shots for Start/End frames</div>
          ...
          {startFrame && !endFrame && (
            <div className="mt-3 flex items-center">
              <input
                ...
                className="w-4 h-4 text-emerald-600 bg-gray-700 border-gray-600 rounded focus:ring-emerald-500 focus:ring-offset-gray-800 cursor-pointer"
              />
              <label
                htmlFor="loop-video-checkbox"
                className="ml-2 text-sm font-medium text-gray-300 cursor-pointer">
                Create a looping product GIF/Video
              </label>
            </div>
          )}
        </div>
    ...
    if (generationMode === GenerationMode.REFERENCES_TO_VIDEO) {
      return (
        <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-wrap items-center justify-center gap-2">
           <div className="w-full text-center text-sm text-gray-400 mb-2">Upload brand assets for reference</div>
  ...
  return (
    <div className="relative w-full">
      {/* Template Selector for SaaS feel */}
      {generationMode === GenerationMode.TEXT_TO_VIDEO && (
        <div className="mb-4">
          <select
            onChange={handleTemplateSelect}
            className="w-full bg-[#2c2c2e] border border-emerald-500/30 text-emerald-400 text-sm rounded-lg p-2.5 focus:ring-emerald-500 focus:border-emerald-500 font-medium">
        ...
        <div className="absolute bottom-full left-0 right-0 mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 shadow-2xl z-20">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <CustomSelect
              ...
              disabled={isRefMode}>
               <option value={VeoModel.VEO}>High Quality (Slower)</option>
               <option value={VeoModel.VEO_FAST}>Fast Turbo</option>
            </CustomSelect>
            <CustomSelect
              label="Format"
              value={aspectRatio}
      ...
      <form onSubmit={handleSubmit} className="w-full">
        {renderMediaUploads()}
        <div className="flex items-end gap-2 bg-[#1f1f1f] border border-gray-600 rounded-2xl p-2 shadow-lg focus-within:ring-2 focus-within:ring-emerald-500/50">
          ...
          <textarea
            ...
            className="flex-grow bg-transparent focus:outline-none resize-none text-base text-gray-200 placeholder-gray-500 max-h-48 py-2 min-h-[50px]"
            rows={1}
          />
          <button
            type="button"
          ...
          <div className="relative group">
            <button
              type="submit"
              className="p-2.5 bg-emerald-600 rounded-full hover:bg-emerald-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-white shadow-emerald-500/20 shadow-lg"
